<div style="display: table;">
	<div id="admin_works">Loading...</div>
	<div id="admin_window" class="bearform">
		...
		<div id="admin_new">
			...
			<select id="createpage_url_prefix">
				<?php
					foreach (explode(',',CATEGORYSET) as $x)
...
<script>
	...
	function getMyWork(workURL) {
		//Refresh work list
		if (typeof workURL == 'undefined') {
			var list = document.getElementById('admin_works');
			while(list.firstChild)
				list.firstChild.remove();
			var refresher = document.createElement('a');
			refresher.textContent = 'Refresh';
			refresher.addEventListener('click',function(){getMyWork();});
			user.view('<?php echo $BW->client['Username']; ?>',function(code,body){
				if (code != 200) {
					var note = document.createElement('span');
					var text = 'Fail to load user work list (' + code + ')';
					note.appendChild(document.createTextNode(text));
					list.appendChild(note);
				}
				else {
					try {
						body.Work.map(function(work){
							var x = document.createElement('a');
							x.textContent = work.Title;
	...
	getMyWork(); //Show all my work after page load
	...
	function xUpload() {
		Array.prototype.slice.call(document.querySelectorAll('.x_upload_preview')).map(function(x){
			...
			ajax('GET','/api/page/create?page='+url,null,function(code,body){
				...
				//Prepare post data
				var post = new Array();
				post['Author']		= '<?php echo $BW->client['Username']; ?>';
				post['Title']		= title;
...
<style>
	...
	#admin_works a:last-child:before {
		content: '₢ '
	}
